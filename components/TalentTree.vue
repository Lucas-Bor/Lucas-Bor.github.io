<template>
  <div class="talent-tree-panel">
    <h3 class="talent-tree-title">
      {{tree.name}}
      <span class="talent-tree-points">({{tree.skillPoints}})</span>
    </h3>
    ...
    <span class="talent-tree-reset" v-on:click="resetTalentTree">Réinitialiser</span>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    getTreeBackgroundImage: function() {
      let backgroundImageUrl =
        ...
        this.constants.backgroundDirectory +
        "background-" +
  ...
  methods: {
    ...
    onIncreaseCurrentSkillTier: function(tier) {
      if (tier > this.tree.currentSkillTier) this.tree.currentSkillTier = tier;
    },
    onDecreaseCurrentSkillTier: function(tier) {
      let totalTierSkillPoints = this.getTotalTierSkillPoints(tier);
      if (totalTierSkillPoints == 0) this.tree.currentSkillTier = tier - 1;
    },
